...
void receive_packet_callback(const void *data, uint16_t len, const linkaddr_t *src, const linkaddr_t *dest) 
{
	...
	if (len == sizeof(nbr_packet) && state == 3) {
	...
	else if (len == sizeof(nbr_packet) && state == 4) {
	...
	} else if (len == sizeof(ack_packet) && state == 5) {
		static ack_packet_struct ack_packet;
...
PROCESS_THREAD(node_a_bonus, ev, data)
{
	...
	while (1) {
		if (state == 0) { // idle state
			...
			state = 2; // proceed to next state if no significant motion detected
